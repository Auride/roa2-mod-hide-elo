/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
/* COMPATIBLE WITH Rivals of Aether II version 1.2.2 -                      */
/* 05-09-2025 - 17811 - [release]                                           */
/* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
// Compile with:
// gcc hideElo-3.2.c -o hideElo-3.2.exe
...
int main(int argc, char **argv) {
	...
	// by a number between 1 and 8 (e.g. "hideElo-3.1r1.exe") it will use the
	...
	// This hides your and your opponent's win streaks. This works by
	// overwriting a conditional jump which is made if a win streak is greater
	// than 0. By nopping out that jump instruction, the game thinks win streaks
	// are always 0 and so doesn't display them. Unfortunately this check isn't
	// used when displaying win streaks after each game, so a separate patch is
	// needed for that.
	aobInject(
		"Hide Win Streak other than after games",
		process, modBase, modEnd,
		"7F 13 ?? 01 48 8B ?? 24 30", +0x0,
		/* nop (2 bytes) */"\x66\x90", 2
	);

	// This changes the first instruction in the function used to show win
	// streaks at the end of games to a return, effectively making the function
	// a nop. I would have nopped out the CALL instruction, but I wasn't able to
	// get a unique AOB pattern for it that wouldn't break with each update,
	// since there seem to be like 5 nearly functions in the game's code to the
	// caller of this function. They're probably for drawing different parts of
	// the UI.
	aobInject(
		"Hide Win Streak after games",
		process, modBase, modEnd,
		"48 89 ?? 24 08 48 89 ?? 24 10 ?? 48 83 EC 20 41 8B ?? 8B ?? 48 8B ?? E8 ?? ?? ?? ?? 48 85 ?? 48 8B ??", +0x0,
		/* ret */"\xC3", 1
	);
