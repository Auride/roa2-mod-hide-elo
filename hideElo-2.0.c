 ...
 *
 * Finally, for `patchBytes`, you will need to see the sequence of bytes
 * corresponding to the assembled machine code in the [ENABLE] section. You can
 * get this either by using the `Disassemble this code region` option and
 * copying the bytes of the injected code, or by using any other x86 assembler
 * software.
 *
...
DWORD64 aobInject(
	...
	printf("\"%s\" patch applied at address 0x%08llX (offset 0x%08llX).\n", description, patchAddr, patchAddr-moduleBase);
	return patchAddr;
}
